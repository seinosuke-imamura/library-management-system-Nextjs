# Next.js 学習メモ（Phase N0〜N6）

図書管理システムを Hono + React(Vite) から **フルスタック Next.js（App Router + Route Handlers）** へリメイクする学習記録。
...
| Phase | 内容 | 状態 |
|-------|------|------|
| N0 | Next.js 基礎（App Router, RSC/Client, Route Handlers, Middleware） | 未着手 |
| N1 | プロジェクト初期化 + Drizzle/DB 層移植 + seed | 未着手 |
| N2 | 書籍 Route Handlers 実装 | 未着手 |
| N3 | JWT + HttpOnly Cookie 認証 + middleware + ロール制御 | 未着手 |
| N4 | 貸出 Route Handlers 実装 | 未着手 |
| N5 | UI 移植（7 ページ + Layout/BookCard） | 未着手 |
| N6 | 全ロール手動テスト + build 確認 + README 整備 | 未着手 |

---

## アーキテクチャ方針

- **フルスタック Next.js**: フロントエンドと API を単一アプリに統合
- **API**: Hono の代わりに Route Handlers（`app/api/**/route.ts`）
- **DB**: Drizzle ORM + SQLite（既存 `backend/src/db/` から移植）
- **認証**: JWT + HttpOnly Cookie + `middleware.ts`（localStorage は使わない）
...
以降、概念的な質問やつまずきはここに Q&A 形式で追記する。
